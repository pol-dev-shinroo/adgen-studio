...
// before. The two exceptions (Part J, the Figma plugin) are GET /api
// /generate/results (list) and GET .../results/:id/figma-export (single
// result) — both fetched from inside a Figma plugin's sandboxed runtime, a
// different, Figma-controlled origin our config can't (and shouldn't) know
// in advance. Confirmed by testing against a real Origin: null header (what
// the plugin's own main-thread sandbox actually sends) that Figma's
// networkAccess manifest field does NOT bypass the target server's own CORS
// check the way it might sound like it does — this server-side opening is
// what actually makes the fetch succeed, not the manifest alone.
//
// Both are read-only/non-mutating/free (no paid API calls, no writes) —
...
const CORS_OPEN_PATHS_RE = /^\/api\/generate\/results(\/[^/]+\/figma-export)?$/
